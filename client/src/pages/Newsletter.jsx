...
async function aiGenerate(){
  ...
  try{
    ...
    const sug = { title: out?.title || 'Newsletter', body: out?.body || '' };
    setSuggestions([sug]);
    setTitle(sug.title);
    setBody(sug.body);
  }catch(err){
...
const translated = await Promise.all(jobs).catch(()=>[]);
      ...
      const payloadBase = { audiences: ['all'], pinned, scheduledAt, expiresAt: expiresAt||null, status, issueDate };
      await Promise.all(setAll.map(v =>
        api.post('/newsletters', { title: v.title, body: v.body, lang: v.code, issueDate, ...payloadBase }).catch(()=>null)
      ));

      const { data: after } = await api.get('/newsletters');
      setRows(after||[]);
      ...
      <div className="rounded-2xl border p-3 dark:border-gray-800">
        ...
        <div className="flex flex-col md:flex-row gap-2">
          <input
            ...
            placeholder="Describe this issue (e.g., August highlights, exam wrap-up, club newsletter...)"
            value={prompt}
      ...
      <div className="rounded-2xl border dark:border-gray-900 p-4">
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            ...
            <input type="date" value={issueDate} onChange={e=>setIssueDate(e.target.value)} className="w-full border rounded px-3 py-2 text-sm bg-white text-gray-900 placeholder:text-gray-400 border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-900 dark:border-gray-800"/>
          </div>
          <div>
          ...
          <div className="md:col-span-2">
            ...
            <textarea rows={8} value={body} onChange={e=>setBody(e.target.value)} className="w-full border rounded px-3 py-2 text-sm bg-white text-gray-900 placeholder:text-gray-400 border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-900 dark:border-gray-800"/>
          </div>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-3 mt-3">
          ...
          <label className="inline-flex items-center gap-2">
            ...
            <span className="text-sm">Pin this newsletter</span>
          </label>
          <div>
          ...
          <div className="sm:col-span-2 flex gap-2">
            ...
            <button onClick={postNewsletter} disabled={posting} className="px-3 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white dark:bg-white dark:text-gray-900">{posting ? 'Posting…' : 'Post'}</button>
          </div>
        </div>
      </div>
            ...
            {rows.map(r=>(
              <li key={r._id} className="rounded-xl border dark:border-gray-900 p-3">
                <div className="flex items-center gap-2">
                  ...
                  <div className="ml-auto text-xs text-gray-500">{r.issueDate ? new Date(r.issueDate).toLocaleDateString() : ''}</div>
                </div>
                <div className="opacity-80 mt-1 whitespace-pre-wrap">{r.body || ''}</div>
              </li>
            ))}
            {(!rows || rows.length===0) && <li className="text-gray-500">No newsletters yet.</li>}
          </ul>
        )}
      </div>
...
function Tones(){ return ['formal', 'friendly', 'celebratory', 'concise']; }
function Lengths(){ return ['short', 'medium', 'long']; }
